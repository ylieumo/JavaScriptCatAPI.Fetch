...
const API_KEY = "live_ejHPDAxyGPayX4OGCibyE9QL3iOdMGtkgx64ovEbhXsjcKQraamHbkElgd5QGfHf";
...
async function initialLoad() {
  try {
    ...
    console.error("Error fetching and populating breeds:", error);
  }
}
...
breedSelect.addEventListener("change", async function () {
    try {
      // Clear previous content in carousel and infoDump
      carousel.innerHTML = "";
      infoDump.innerHTML = "";
  
      // Retrieve information on the selected breed from the cat API
      const selectedBreedId = breedSelect.value;
      const response = await fetch(`https://api.thecatapi.com/v1/images/search?limit=10&breed_ids= ${selectedBreedId } &api_key=${API_KEY};
      ...
      // Create and append informational section in the infoDump element
      const breedName = breedSelect.options[breedSelect.selectedIndex].text;
      const infoText = document.createElement("p");
      infoText.textContent = `You selected: ${breedName}`;
  ...
  // (assuming an initial selection is made when the page loads)
